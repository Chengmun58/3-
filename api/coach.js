...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ provider: process.env.GEMINI_API_KEY ? "gemini" : "openai", result });
  } catch (error) {
    console.error("AI coach error", error);
    return res.status(500).json({ error: error.message || "AI 코치 연결 중 오류가 발생했습니다." });
  }
}
